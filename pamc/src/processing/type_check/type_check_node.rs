...
fn get_type_of_call_dirty(
    ...
    match call.arg_list_id {
        NonEmptyCallArgListId::Unlabeled(arg_list_id) => {
            get_type_of_unlabeled_call_dirty(state, call_id, call.callee_id, arg_list_id)
        }
        NonEmptyCallArgListId::UniquelyLabeled(arg_list_id) => {
            get_type_of_labeled_call_dirty(state, call.callee_id, arg_list_id)
        }
    }
}

fn get_type_of_unlabeled_call_dirty(
    state: &mut State,
    call_id: NodeId<Call>,
    callee_id: ExpressionId,
    arg_list_id: NonEmptyListId<ExpressionId>,
) -> Result<NormalFormId, Tainted<TypeCheckError>> {
    let callee_type_id = get_type_of_expression_dirty(state, None, callee_id)?;
    let callee_type_id = if let ExpressionId::Forall(id) = callee_type_id.raw() {
        id
    } else {
        return tainted_err(TypeCheckError::IllegalCallee(callee_id));
    };
    let arg_ids = state.registry.get_list(arg_list_id).to_vec();
    ...
    let NonEmptyParamListId::Unlabeled(callee_type_param_list_id) = callee_type.param_list_id else {
        return tainted_err(TypeCheckError::LabelednessMismatch {
            call_id,
        });
    };
    
    let (callee_type_param_name_ids, callee_type_param_type_ids) = get_names_and_types_of_unlabeled_params(state, callee_type_param_list_id);
...
fn get_type_of_labeled_call_dirty(
    state: &mut State,
    callee_id: ExpressionId,
